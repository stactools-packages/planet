...
You'll need the `stactools-planet` and `stactools-browse` packages to follow along. You can install these with:

.. code-block:: console

   > pip install stactools-planet stactools-browse
...
You'll need the `planet CLI <https://planetlabs.github.io/planet-client-python/cli/index.html>`_ installed, as well as the ``stactools-planet`` and ``stactools-browse`` package.
